...
q = dbInit('Server','spyhop.ucsd.edu','Port',9779); % setup query handler
...
[odir, ~, ~] = fileparts(which('XMLautodet_dir'));
...
idir{ii} = REMORA.dt_bwb.indir;

PARAMS.outpath = REMORA.dt_bwb.outdir;
...
xml_out = sprintf(sprintf('%s_kern%s_thresh%d.xml',dataID,REMORA.dt_bwb.kern,REMORA.dt_bwb.thresh)); 
